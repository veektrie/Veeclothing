...
const Navbar = () => {
  ...
  return (
    <>
      <header
        className="fixed top-3 left-0 right-0 z-[1000] px-4 md:px-8 pointer-events-none"
      >
        <div
          ...
          <nav className="hidden md:flex items-center gap-8">
            {navLinks.slice(1).map((link) => (
              <Link
                key={link.label}
                href={link.href}
                className="text-[9px] font-metro uppercase tracking-[0.35em] text-white/50 hover:text-[#D4AF37] transition-all duration-500 hover:tracking-[0.45em]"
              >
                {link.label}
              </Link>
            ))}
