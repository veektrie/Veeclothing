...
const FloatingTrustBar = () => {
  ...
  return (
    <div style={{ display: 'flex', justifyContent: 'center', marginBottom: 60 }}>
      {/* Outer pill — flex row, label fixed on left, marquee clips on right */}
      <div style={{
        display: 'flex',
        alignItems: 'stretch',
        ...
        <div style={{
          ...
          display: 'flex',
          alignItems: 'center',
          padding: '12px clamp(0.75rem, 3vw, 1.75rem)',
        ...
        {/* Scrolling marquee cell */}
        <div style={{ flex: 1, overflow: 'hidden', position: 'relative' }}>
          {/* Right fade mask */}
          <div className="hidden md:block" style={{ position: 'absolute', top: 0, bottom: 0, right: 0, width: 80, zIndex: 10, pointerEvents: 'none', background: 'linear-gradient(to left, rgba(248,250,252,1), transparent)' }} />
          ...
          >
            {doubled.map((client, i) => (
              <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '12px 25px', borderRight: '1px solid rgba(0,0,0,0.05)' }}>
...
export default function WhyChooseUs() {
  return (
    <section className="why-choose-section" style={{ padding: 'clamp(60px, 10vw, 100px) 0', overflow: 'hidden' }}>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
          {cards.map((card, i) => {
            ...
            return (
              <motion.div
                key={card.title}
                className={`${card.cols === 2 ? 'sm:col-span-2 lg:col-span-2' : 'col-span-1'} why-choose-card`}
                style={{
